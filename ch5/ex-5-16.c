...
int main(int argc, char *argv[]) {
  ...
  if ((nlines = readlines(lineptr, MAXLINES, lines)) >= 0) {
    quick_sort((void **)lineptr, 0, nlines - 1,
               (int (*)(void *, void *))(
                   numeric     ? reverse ? rnumcmp : numcmp
                   : reverse   ? fold        ? directory ? foldreversedirectorycmp
                                                         : foldresversestrcmp
                                 : directory ? reversedirectorycmp
                                             : rstrcmp
                   : fold      ? directory ? folddirectorycmp : foldstrcmp
                   : directory ? directoryycmp
                               : strcmp));
